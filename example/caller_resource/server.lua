...
local function fetchAndFormatUser(userId)
    local user = fx.invokeExportUnwrap('example_resource', 'getUser', userId)
    return fx.invokeUnwrap(formatUser, user)
end

CreateThread(function()
    Wait(1000)  -- Ensure example_resource is loaded

    -- === EXPORTS (cross-resource) ===

    -- Method 1: invokeExportUnwrap - get value or throw with real error message
    local ok, user = pcall(fx.invokeExportUnwrap, 'example_resource', 'getUser', 42)
    if ok then
    ...
    local result = fx.invokeExport('example_resource', 'getUser', 0)
    ...
    -- Method 3: invokeExport - divide (message only; metadata in errorMeta)
    local divResult = fx.invokeExport('example_resource', 'divide', 10, 0)
    if fx.isErr(divResult) then
        print('[caller] Divide error:', divResult.error)
        if divResult.errorMeta then
            print('[caller]   (resource:', divResult.errorMeta.resource, 'line:', divResult.errorMeta.line, ')')
        end
    end

    -- Method 4: invokeExport - nested export (getUserWithBalance internally uses fx.invoke)
    local balanceResult = fx.invokeExport('example_resource', 'getUserWithBalance', 42)
    ...
    local ok, formatted = pcall(fx.invokeUnwrap, fetchAndFormatUser, 42)
    if ok then
        print('[caller] Nested local+export:', formatted)
    else
        print('[caller] Nested error:', formatted)
    end

    -- Method 7: Raw invocation - unchanged behavior
    local user = exports['example_resource']:getUser(42)
